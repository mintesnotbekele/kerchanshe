...
export const getDataJabatan = async (req, res) => {
    try {
        ...
        if (req.hak_akses === "admin") {
            ...
            if (req.userId !== DataJabatan.userId) return res.status(403).json({ msg: "Access prohibited" });
...
export const getDataJabatanByID = async (req, res) => {
    try {
        ...
        if(response){
            ...
            res.status(404).json({msg: 'Job data with that ID was not found'});
        }
    } catch (error) {
        res.status(500).json({msg: error.message});
    }
...
export const deleteDataJabatan = async (req, res) => {
    try {
        ...
        if (req.hak_akses === "admin") {
            ...
            if (req.userId !== jabatan.userId) return res.status(403).json({ msg: "Access Prohibited" });
            await jabatan.destroy({
                where: {
                    [Op.and]: [{ id_jabatan: jabatan.id_jabatan }, { userId: req.userId }]
                },
