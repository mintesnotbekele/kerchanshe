        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(deleteDataKehadiran(id)).then(() => {
                    Swal.fire({
                        title: 'Succeed',
                        text: 'Attendance data was successfully deleted.',
                        icon: 'success',
                        timer: 1000,
                ...
                <div className="bg-gray-2 text-left dark:bg-meta-4 mt-6">
                    {filteredDataKehadiran.slice(startIndex, endIndex).reduce((uniqueEntries, data) => {
                        ...
                        <h2 className="px-4 py-2 text-black dark:text-white" key={`${data.bulan}-${data.tahun}`}>
                            Displaying Employee Attendance Data Month :
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Displaying {startIndex + 1}-{Math.min(endIndex, filteredDataKehadiran.length)} dari {filteredDataKehadiran.length} Employee Attendance Data
                        </span>
                    </div>
